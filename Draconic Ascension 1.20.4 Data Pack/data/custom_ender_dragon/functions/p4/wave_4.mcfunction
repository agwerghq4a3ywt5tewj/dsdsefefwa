bossbar set minecraft:phase_4_mobs name "Wave 4"
bossbar set minecraft:phase_4_mobs max 2

scoreboard players set @e[type=marker,tag=master] dragon_phase 10

execute as @a[nbt={Dimension:"minecraft:the_end"}] at @s run playsound minecraft:block.beacon.power_select master @a ~ ~ ~ 2 2

summon warden ~ ~ ~ {Tags:["phase_4_mob"],Brain:{memories:{"minecraft:dig_cooldown":{value:{},ttl:1200L}}},Health:250f,DeathLootTable:"minecraft:empty"}
summon warden ~ ~ ~ {Tags:["phase_4_mob"],Brain:{memories:{"minecraft:dig_cooldown":{value:{},ttl:1200L}}},Health:250f,DeathLootTable:"minecraft:empty"}
